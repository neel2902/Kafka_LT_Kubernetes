...
## More than 10 arguments not passed 
# param_host="-GbootstrapServers=${bootstrapServers} -GsecurityProtocol=${securityProtocol} -GsaslJaasConfig=${saslJaasConfig} -GsaslMechanism=${saslMechanism} -Gclient.id=${clientId} -GclientDnsLookup=${clientDnsLookup}"
param_host="-GbootstrapServers=${bootstrapServers}"
param_test="-GsessionCount=${sessionCount} -Gduration=${duration}"
param_user="-GtargetThroughput=${targetThroughput} -Gmessage.size=${messageSize} -Gtopic=${topic}"
...
{ 
    ...
    echo "jmeter ${param_host} ${param_test} ${param_user} ${report_command_line} --logfile /report/${jmx}_$(date +"%F_%H%M%S").jtl --nongui --testfile ${jmx} -Dserver.rmi.ssl.disable=true --remoteexit --remotestart ${slave_list} >> jmeter-master.out 2>> jmeter-master.err &"
    echo "trap 'kill -10 1' EXIT INT TERM"
